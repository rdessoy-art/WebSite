...
- [x] **P2 — No `loading="lazy"` on below-the-fold images.** **Done 2026-07-10** for the `<img>` tags: index.html's 13 sponsor-grid logos, events.html's 11 country-flag + 2 venue-logo images, merchandise.html's 2 rear/back carousel photos (never visible without user interaction, so zero LCP risk — the front-facing product photos were deliberately left eager). **Not applicable** to the "social feed cards" originally named in this item: those (and the News blog cards) render via CSS `background-image` on a `<div>`, not `<img>` tags — the HTML `loading` attribute only works on `<img>`/`<iframe>`. Making those lazy would need restructuring to real `<img>` elements or an IntersectionObserver — left as a separate, smaller future item if it's worth the churn.
...
- [x] **P2 — Newsletter popup markup was pasted inline into 5 pages.** **Done 2026-07-10:** added `renderNewsletterPopup()` to `newsletter-popup.js`, which injects the modal HTML via `insertAdjacentHTML`; all 5 pages now call it instead of hardcoding the 28-line block. Also found and fixed extra drift while doing this: `index.html` didn't include the shared `newsletter-popup.css`/`.js` files at all — it had its own full inline copies of the CSS (34 selectors) and JS, and the CSS copy was missing a `z-index: 1` rule the shared file has, while the JS copy had a stray unused variable and lacked null-guards the shared version has. index.html now uses the same shared files as the other 4 pages. Verified via jsdom (in an isolated Node process per page, after discovering shared-process runs cross-contaminate) that the popup renders with all expected elements, `showNewsletterPopup()`/`closeNewsletterPopup()` work, and 7-day dismissal is stored correctly on all 5 pages. `newsletter-popup.html` remains the (now genuinely accurate) reference copy.
...
- [x] **P2 — The hamburger menu was a `<div onclick>`** on the 5 pages with the full nav (index, events, fanclub, merchandise, partnership-menu). **Done 2026-07-10:** converted to `<button type="button" aria-label="Menu" aria-expanded="false" aria-controls="nav">`, added a small CSS reset (`background/border: none`) so it renders identically to the old div, and `toggleMenu()`/`closeMenu()` in `nav.js` now keep `aria-expanded` in sync. Verified via Node DOM simulation (browser extension was disconnected this session) that the attribute flips correctly through open/close/re-open/programmatic-close.
...
- [x] **P3 — Internal links inconsistently opened new tabs.** **Done 2026-07-10:** removed the stray `target="_blank"` from the `sponsor-martin-sheath.html` hero-strip link — now matches every other internal sponsor link (same tab, `rel="noopener"` kept).
- [x] **P3 — No "skip to content" link.** **Done 2026-07-10:** added to the 5 full-nav pages (index, events, fanclub, merchandise, partnership-menu), visually hidden until keyboard-focused. On index.html specifically, target is `#home` rather than a new `#main-content` id — that page toggles section visibility via a `section.active { display: block }` / `section { display: none }` pair driven by `showSection()`, and a generic `<main id="main-content">` would have collided with that (hash-loading `#main-content` would hide every section with nothing to replace them, i.e. a blank page). `#home` is the existing, already-correct target. Verified via a Node DOM simulation of `showSection('home')`.
...
- [x] **P2 — `STRIPE_SETUP.md` described a flow that no longer exists.** **Done 2026-07-10:** full rewrite describing the real flow (direct Payment Links, how to create one, how the merchandise color-variant `buyX()`/`changeXImage()` functions work) — verified the referenced function names against the current `merchandise.html`.
- [x] **P2 — `_internal/CLAUDE_CODE_GUIDE.md` was stale.** **Done 2026-07-10:** full refresh — 13 sponsor pages, `nav.js`/`footer.js`, favicon, feed-security note, accessibility section, current image inventory (verified every referenced filename exists), updated Known Issues to only what's still genuinely open.
- [x] **P3 — `SITEMAP_MAINTENANCE.md`** re-checked. **Done 2026-07-10:** page list now matches `sitemap.xml` exactly (was 6 months stale — missing 4 sponsor pages, still listed success/cancel which are now excluded+noindexed instead); also removed a stale "excluded files" reference to a path (`google-apps-script/...`) that doesn't exist anywhere in this repo.
...
3. ~~**Rolling:** doc refresh (§7) · hamburger a11y + h1s + noopener sweep · the `reportModal` bug · skills as you next need each workflow.~~ **Done 2026-07-10** — reportModal fixed, 16 pages got their missing `<h1>` (uncovering that all 13 sponsor pages had unused hero CSS waiting for one), 37 links got `rel="noopener"`, hamburger menu is a real `<button>` now, and all three stale docs (STRIPE_SETUP.md, the dev guide, SITEMAP_MAINTENANCE.md) are refreshed to match reality.
4. **What's left (see individual sections above for detail):** performance (`loading="lazy"`, image `width`/`height`, remaining oversized non-merch photos) · newsletter-popup.js duplication cleanup · CSS consolidation into a shared stylesheet · the smaller P3 accessibility/polish items (skip-to-content link, internal-links-in-new-tab consistency, inline `onclick` direction-of-travel) · Publisher-side feed sanitization (§6) · repo housekeeping (`.git` size, PDFs in root, emoji folder names, blog archive navigation).
